...
Algorytm SVM jest wykorzystywany do klasyfikacji obserwacji w uczeniu maszynowym.
Obserwacje zostają rozmieszczone w przestrzeni wielowymiarowej,
algorytm ma za zadanie znalezienie hiperpłaszczyzny rozdzielającej obserwacje należące do dwóch klas.
Jest więc klasyfikatorem binarnym. 
...
Hiperpłaszczyzna \ang{hiperplane} jest $n-1$ wymiarowa i zgodnie z definicją rozdziela przestrzeń na dwie części.
Poszukiwana hiperpłaszczyzna powinna być hiperpłaszczyzną optymalną \ang{optimal hiperplane}, 
to znaczy powinna posiadać możliwe największy margines - czyli odległość od najbliższych obserwacji należących do różnych klas.
\cite{Cortes1995}

Biorąc przestrzeń dwuwymiarową, możemy ją osadzić na płaszczyźnie. 
...
jest marginesem pomiędzy hiperpłaszczyzną, a obserwacjami
...
\TODO{Jak hiperpłaszczyzna jest wyszukiwana?}
...
\subsection{GMM}
Modele Mieszane Gaussa \ang{Gaussian Mixture Models} jest to model probabilistyczny.
Przedstawia podgrupy z całości obserwacji.
Podgrupy mają rozkład normalny.
Modele mieszane nie wymagają informacji jaką podgrupę reprezentuje dana część populacji.
Model rozpoznaje istnienie podgrupy wykorzystując cechy rozkładu statystycznego.
Dzięki temu tworzenie modeli rozpoznających podgrupy może odbywać się w sposób automatyczny i nienadzorowany.

Przykładowo wzrost człowieka może zostać przedstawiony za pomocą Mieszanych Modeli Gaussa.
W populacji ludzi można wyróżnić dwie grupy - kobiet i mężczyzn,
gdzie statystyka rozkładu wzrostu w populacji mężczyzn i populacji kobiet przestawia się rozkładem normalnym dla każdej  z grup z osobna.
Rozkład wzrostu dla ludzi w ogólności nie jest rozkładem normalnym,
ponieważ jest sumą dwóch rozkładów normalnych o różnej wartości średniej i odchyleniu standardowym.
Przykładowo średni wzrost w Polsce wynosi odpowiednio 178,7cm dla mężczyzn i 163,7cm dla kobiet \cite{Kuaga2011}
Stosując Mieszane modele Gaussa dla powyższego przypadku otrzymamy dwa komponenty.
Pierwszy komponent będzie modelował rozkład normalny wzrostu w populacji mężczyzn,
drugi rozkład normalny wzrostu w populacji kobiet.

Rozkład Gaussa jest najbardziej spodziewanym rozkładem prawdopodobieństwa  w przyrodzie.
Niezależnie od wielkości badanej populacji występujące wartości średnie będą zbliżone, 
a wartości skrajne będą występowały odpowiednio rzadziej. 
Jedną z przesłanek sugerującą zasadność wykorzystania GMM jest występowanie wielu wierzchołków w rozkładzie.
Próby przybliżania rozkładu wielowierzchołkowego będącego sumą wielu rozkładów normalnych nie da zadowalających rezultatów.

Mieszany model Gaussa definiują dwa typy parametrów: waga komponentu, oraz jego średnia i wariancja lub kowariancja.
Dla $k$-elementowego zbioru komponentów, komponent $k$ty posiada średnią $\mu_k$ i wariancję $\sigma_k$.
Dla przypadku wielowymiarowego $k$ty komponent posiada wektor średnich$\vec{\mu_k}$ oraz macierz kowariancji $\Sigma_k$.
Wagę $k$tego komponentu $C_k$ oznacza się symbolem $\phi_k$. 
Wagi komponentów muszą spełniać warunek $\sum_{i=1}^{K}\phi_i = 1$.
...
\begin{figure}[hc]
	\begin{align}
		...
		\sum_{i=1}^K\phi_i &= 1
	\end{align}
	\caption{Mieszany model Gaussa dla jednego wymiaru}
\end{figure}
\begin{figure}[hc]
	\begin{align}
	...
	\sum_{i=1}^K\phi_i &= 1
\end{align}
\caption{Mieszany model Gaussa dla wielu wymiarów}
\end{figure}

Zadana jest liczba komponentów.
Do znalezienia parametrów modeli najczęściej wykorzystuje się metodę maksymalizacji oczekiwań 
\ang{expectation maximization}.
Zazwyczaj do trenowania modelu używa się przybliżenie największego podobieństwa
\and{maximum likelihood estimation} poszukującą największego podobieństwa obserwacji i trenowanych modeli.
Algorytm maksymalizacji oczekiwań pozwala znaleźć przybliżenie największego podobieństwa dla parametrów modelu przy niekompletnych danych, 
brakujących lub ukrytych obserwacjach.
Jest metodą iteracyjną przybliżającą funkcję największego podobieństwa. 
Przybliżenie największego podobieństwa pozwala znaleźć najlepsze dopasowanie dla zbioru danych,
ale nie jest w stanie dla niekompletnych zbiorów danych.
Maksymalizacja oczekiwań, 
jako algorytm bardziej złożony,
jest w stanie znaleźć parametry modelu, nawet jeżeli badany zbiór danych nie jest kompletny.
Brakujące wartości zastępowane są losowymi,
następnie z ich wykorzystaniem znajdowane jest przybliżenie drugiego zbioru danych.
